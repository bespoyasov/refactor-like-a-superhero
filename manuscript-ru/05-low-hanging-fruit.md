...
Кроме возможностей языка полезно помнить о функциях и типах данных из стандартной библиотеки и встроенных API.[^rightstructure] Например, во фрагменте кода ниже мы можем упростить самописную сериализацию формы с помощью `FormData`:

```js
// Ручной обход каждого поля с получением значения:
...
| Уточнение 🤓                                                                                                                                                                                                    |
| :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Бывают случаи, когда стандартные API для решения задачи не подходят, и мы вынуждены писать велосипед. В этих случаях нам стоит зафиксировать в документации причины, по которым стандартные решения не подошли. |
