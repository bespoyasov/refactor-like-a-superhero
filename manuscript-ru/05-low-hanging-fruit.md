...
Если мы используем гит «тактически» и проверяем, что изменилось с последнего коммита, то можем заметить эту ошибку в диффе:

<figure>
  <img src="../images/05-tactical-git.png" width="800">
  <figcaption><em>Гит показывает, к каким именно изменениям привело форматирование</em><br><br></figcaption>
</figure>

...Но искать такие ошибки вручную ненадёжно, поэтому их поиск тоже лучше автоматизировать.

Удобнее всего покрыть код надёжным набором тестов перед началом работы. Если мы запустим тесты в интерактивном режиме рядом с редактором, то будем сразу же видеть, какие ошибки появились после применения форматирования.
...
Чтобы этого избежать, мы снова можем использовать «тактический» гит. Для этого пробежимся по изменениям с последнего коммита и проверим, что именно переименовалось и как:
