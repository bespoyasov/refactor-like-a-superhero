...
Успешному рефакторингу большого куска приложения может мешать слабая архитектура. Сильное зацепление между частями кода и беспорядочные зависимости мешают находить швы и изолировать изменения во время работы с кодом. А неочевидное взаимодействие частей приложения делает код запутанным и непонятным.
...
Например, в интернет-магазине для создания заказа сервер просит от в запросе клиента объект с полем `products`. Так совпало, что в клиентском коде объект заказа тоже содержит поле `products`. Появляется желание использовать этот объект напрямую при отправке заказа:
...
![При использовании их общение модулей сводится к соединению адаптеров с портами других модулей](../images/15-ports-adapters.png)
