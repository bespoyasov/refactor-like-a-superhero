...
<figure>
  <img src="../images/18-refactoring-ping-pong.png" width="800" alt="Последовательность изменений кода на диаграмме проходит зигзагом между кодом приложения и тестовым кодом">
...
В отличие от моков _стабы и простые тестовые данные_ помогают писать более устойчивые к изменениям тесты.[^testingprinciples] Стабы и тестовые данные прощают нам при их использовании гораздо больше, чем требуют. Они помогают избегать урона от тестов и тратить меньше времени на их обновление.
...
Например, бизнес-логику мы можем тестировать без моков в принципе.[^codethatfits][^testingprinciples] Это сложно, когда логика перемешана с эффектами; но проще, если она описана чистыми функциями.
...
Выделенные эффекты можно протестировать отдельно интеграционными или E2E тестами. В зависимости от того, как в нашем проекте устроена работа зависимостями типа `fs`, нам может быть достаточно протестировать только адаптеры к ним. Как правило, сложность и требовательность моков в таком случае будет ниже.
...
Если сложность функции равна единице, а тест приносит больше дополнительной работы, чем реальной пользы, то от теста можно отказаться.[^testingprinciples] Например, отдельный юнит-тест для функции `fullName` может быть лишним:
