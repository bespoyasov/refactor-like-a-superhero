...
```js
// Например, если корзина валидна, но пуста,
// компонент отрендерит пустой список <ul></ul>:

const validEmptyCart = [];
<CartProducts items={validEmptyCart} />;
```

В коде выше состояние «Пустой корзины» валидно, но представляет собой крайний случай. Функциональный пайплайн делает такие состояния заметнее, потому что они выбиваются из «нормального» выполнения кода. А чем более заметны крайние случаи, тем раньше мы их обнаружим и обработаем:
...
Такой «отсев» помогает нам обнаружить больше потенциальных крайних случаев и сделать это раньше. Благодаря этому программа становится надёжнее и точнее описывает предметную область.

| 👀 К слову                                                                                    |
| :-------------------------------------------------------------------------------------------- |
| Более детально об «отсеве» крайних случаев мы поговорим в главе об условиях и сложности кода. |
