...
![«Островки» отвечают за конкретные задачи предметной области и общаются друг с другом по «мостикам» публичного API, событий или сообщений](../images/13-coupling-cohesion.png)
...
```ts
// cart.ts
// Выносим проверку пустоты корзины
// в отдельный модуль `cart`:
...
![Общение сводится к передаче и получению сообщений от передатчика](../images/13-message-bus.png)
